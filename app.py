...
<p style="color:#64748b;margin-bottom:20px;">Base de datos con 90+ partidas oficiales del Ministerio de Transportes</p>
...
function cargar(){
  ...
  const f=c?DB.filter(x=>x.cat===c):DB;
  s.innerHTML='';
  f.forEach(x=>{
    const o=document.createElement('option');
    o.value=x.cod;
    o.textContent=x.cod+' - '+x.nom+' ('+x.u+') S/ '+x.p.toFixed(2);
    s.appendChild(o);
  });
}

document.getElementById('cat').onchange=cargar;

document.getElementById('buscar').oninput=function(){
  const q=this.value.toLowerCase();
  if(q.length<2){cargar();return;}
  const s=document.getElementById('sel');
  const r=DB.filter(x=>x.nom.toLowerCase().includes(q)||x.cod.toLowerCase().includes(q));
  s.innerHTML='';
  r.forEach(x=>{
    const o=document.createElement('option');
    o.value=x.cod;
...
function actualizar(){
  ...
  const ut=d*0.08;
  const t=d+gg+ut;
  document.getElementById('td1').textContent='S/ '+d.toFixed(2);
  document.getElementById('td2').textContent='S/ '+gg.toFixed(2);
  document.getElementById('td3').textContent='S/ '+ut.toFixed(2);
  document.getElementById('td4').textContent='S/ '+t.toFixed(2);
  document.getElementById('cd').textContent='S/ '+Math.round(d).toLocaleString('es-PE');
  document.getElementById('tot').textContent='S/ '+Math.round(t).toLocaleString('es-PE');
  document.getElementById('num').textContent=pres.length;
}
...
function csv(){
  ...
  let d=0;
  pres.forEach(x=>{
    c+=x.cod+',"'+x.nom+'",'+x.u+','+x.met+','+x.p+','+x.par+'\n';
    d+=x.par;
  });
